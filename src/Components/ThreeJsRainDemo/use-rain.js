...
const RAIN_COLORS = ['#cdd1d3', '#666869']
...
export const useRain = ({ angle = -45, count = 100 }) => {
  // raindrop starting position
  ...
  // radindrop come from which angle
  ...
  const lines = useMemo(() => {
    return Array(count)
      ...
      .map(() => {
        // h / deltax = tan(ang)
        // 直角边
        const leg = Math.random() * 0.1
        // FIXME: should modify from angle
        const orientation = random.inRange(comefrom[angle2placement(_angle)])
